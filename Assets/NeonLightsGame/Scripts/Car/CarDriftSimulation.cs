...
public class CarDriftSimulation : MonoBehaviour
{
    ...
    {
        if (collision.transform.root.GetComponent<CarDriftSimulation>())
        {
            collision.transform.root.GetComponent<CarDriftSimulation>().slip2 = Mathf.Clamp(collision.relativeVelocity.magnitude, 0.0f, 10.0f);
    ...
    {
        if (collision.transform.root.GetComponent<CarDriftSimulation>())
            collision.transform.root.GetComponent<CarDriftSimulation>().slip2 = 5.0f;
    ...
    void FixedUpdate()
    {
        ...
        if (true /*vehicleMode == VehicleMode.Player && GameUI.manage.gameStarted*/)
        {
            if (true /*AIControl.manage.controlMode == ControlMode.Simple*/)
            {
                accel = 0;
                brake = false;
                shift = false;
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && (currentGear > 1 && speed > 50.0f) && shifmotor)
            {
                ...
                //carParticles.shiftParcle1.emissionRate = Mathf.Lerp(carParticles.shiftParticle1.emissionRate, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
                //carParticles.shiftParcle2.emissionRate = Mathf.Lerp(carParticles.shiftParticle2.emissionRate, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
            }
            else
            {
